...
f=F/.x->y;z=Series[f,{y,0,1}]//Normal//Simplify;
z/.y->x//Simplify]
S[z_]:=z//Simplify
...
Do[Do[hh[i,j]=HH[[i,j]];gg[i,j]=mtr[[i,j]],{j,3}],{i,3}];

Do[Do[Do[cs1[i,j,k]=D[gg[i,j],x[k]]+D[gg[i,k],x[j]]-
D[gg[j,k],x[i]]//Simplify,{k,3}],{j,3}],{i,3}];
Do[Do[Do[cs2[i,j,k]=Sum[hh[i,s] cs1[s,j,k]/2,{s,3}]//Simplify,
{k,3}],{j,3}],{i,3}];
Do[cs[i]=Sum[cs2[j,i,j],{j,3}]//Simplify,{i,3}];

Do[Do[rici[i,j]=Sum[D[cs2[k,i,j],x[k]],{k,3}]-D[cs[i],x[j]]+Sum[Sum[cs2[k,i,j]cs2[l,k,l]-cs2[k,l,i]cs2[l,k,j]//Simplify,{l,3}],{k,3}],{j,3}],{i,3}];
...
HT=hh[1,1]hh[2,2](ee[1,2]^2-ee[1,1]ee[2,2])+
hh[1,1]hh[3,3](ee[1,3]^2-ee[1,1]ee[3,3])+hh[2,2]hh[3,3](ee[2,3]^2-ee[2,2]ee[3,3]);
(HT gd-Ub/gd)/4//S
]


Killing[Vels_]:=Module[{DG,ss},
Do[Do[kil[i,j]=Sum[D[Vels[[s]],x[i]]gg[s,j]+D[Vels[[s]],x[j]]gg[i,s]+Vels[[s]]D[gg[i,j],x[s]],
{s,3}],{j,3}],{i,3}];
Array[kil,{3,3}]//Simplify
]
Commut[vel1_,vel2_]:=
Module[{},
Do[com[i]=Sum[vel1[[s]]D[vel2[[i]],x[s]]-vel2[[s]]D[vel1[[i]],x[s]],
{s,1,3}],{i,1,3}]//Simplify;
...
rrr[2]=(D[vl[3],x[1]]-D[vl[1],x[3]])/gd//Simplify;rrr[3]=(D[vl[1],x[2]]-D[vl[2],x[1]])/gd//Simplify;
Array[rrr,3]
]
...
Do[Do[Do[dgd[i,j,k]=D[h[[k,i]],x[j]]+D[h[[k,j]],x[i]]-D[h[[i,j]],x[k]]-
2Sum[cs2[s,i,j]h[[k,s]],{s,3}]
//Simplify,{k,3}],{j,3}],{i,3}];
Do[Do[Do[dg[i,j,k]=Sum[hh[i,s]dgd[j,k,s]/2,{s,3}]//Simplify,{k,3}],{j,3}],{i,3}];
Do[ds[i]=Sum[dg[k,i,k]//Simplify,{k,3}],{i,3}];
Do[Do[dR[i,j]=-D[ds[i],x[j]]+Sum[D[gd dg[k,i,j],x[k]]/gd+cs2[k,i,j]ds[k]//Simplify,{k,3}]-
...
Do[Do[cov[i,j]=D[Vels[[i]],x[j]]+Sum[cs2[i,j,k]Vels[[k]],
{k,3}],{j,3}],{i,3}];
Array[cov,{3,3}]//Simplify
]
Geodes[Field_]:=Module[{DG,ss},
Do[zz[i]=Sum[Field[[k]](D[Field[[i]],x[k]]+Sum[cs2[i,j,k]Field[[j]],{j,3}]),{k,3}]//Simplify,{i,3}];
ZZ=Array[zz,3]
]
...
Do[div[j]=((Sum[D[gd tu[i,j],x[i]]/gd,{i,3}])-(Sum[Sum[cs2[i,j,k]tu[k,i],{k,3}],{i,3}]))//Simplify,{j,3}];
Array[div,3]
]
KillT[V_,T_]:=Module[{i,j,k},
Do[Do[kT[i,j]=Sum[V[[k]]D[T[[i,j]],x[k]]+D[V[[k]],x[i]]T[[k,j]]+D[V[[k]],x[j]]T[[i,k]]
,{k,3}]//Simplify,{j,3}],{i,3}];
Array[kT,{3,3}]
]
RR[vec_]:=Rot[Rot[vec]]-m vec//Simplify
KillV[V_,A_]:=Module[{i,k},
Do[kV[i]=Sum[V[[k]]D[A[[i]],x[k]]-D[V[[i]],x[k]]A[[k]],{k,3}]//Simplify,{i,3}];
Array[kV,3]
]
...
Do[b[i]=Sum[Sum[Sum[hh[k,j](cs2[i,j,l]D[A[[l]],x[k]]+
cs2[i,k,l]D[A[[l]],x[j]]+
(D[cs2[i,j,l],x[k]]+(Sum[cs2[i,k,s]cs2[s,j,l]-
cs2[i,l,s]cs2[s,j,k],{s,3}]))A[[l]]),{l,3}],{k,3}],{j,3}]//Simplify,{i,3}];
B=Array[b,3];
W+B+m A//Simplify
]
LapT[h_]:=Module[{mh,nh},
Do[Do[Do[mh[i,j,k]=D[h[[j,k]],x[i]]-Sum[(cs2[s,i,j]h[[s,k]]+cs2[s,i,k]h[[j,s]]),{s,3}]//Simplify,{k,3}],{j,3}],{i,3}];
Do[Do[Do[nh[i,j,k]=Sum[hh[i,s]mh[s,j,k],{s,3}]//Simplify,{k,3}],{j,3}],{i,3}];
Do[Do[lt[j,k]=Sum[D[nh[i,j,k]gd,x[i]]/gd-Sum[(cs2[s,i,j]nh[i,s,k]+cs2[s,i,k]nh[i,j,s]),{s,3}],{i,3}]//Simplify,{j,3}],{k,3}];
Array[lt,{3,3}]
]
