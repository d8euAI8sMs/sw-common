...
Decart := Ricci[{ct,x,y,z}, Make4Metrics[GGD]]
...
Ricci[coords_,mtr_] := Module[{DG,ss,i,j,k,s,zzzz},
	...
	Do[Do[Do[
		cs1[i,j,k] = D[gg[i,j], x[k]] + D[gg[i,k], x[j]] - D[gg[j,k], x[i]] //Simplify,
			{k,0,3}], {j,0,3}], {i,0,3}];
	Do[Do[Do[
		cs2[i,j,k] = Sum[hh[i,s] cs1[s,j,k] / 2, {s,0,3}] //Simplify,
			{k,0,3}], {j,0,3}], {i,0,3}];
	Do[cs[i] = Sum[cs2[j,i,j], {j,0,3}] //Simplify, {i,0,3}];
	...
	Do[
		Do[
			rici[i,j] =
				...
				D[cs[i], x[j]] +
				Sum[
					Sum[cs2[k,i,j] cs2[l,k,l] - cs2[k,l,i] cs2[l,k,j] //Simplify, {l,0,3}],
					{k,0,3}
				],
				{j,0,3}
		],
	...
	Do[Do[ric[i,j] = Sum[hh[i,s]rici[s,j] //Simplify, {s,0,3}], {j,0,3}], {i,0,3}];
	rs = Sum[ric[i,i], {i,0,3}] //Simplify;
	Rij = Array[ric,{4,4},{{0,3},{0,3}}]
]
